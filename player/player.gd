class_name Player

extends CharacterBody2D

@export_category("Sword")
@export var sword_damage: int = 2
@export_category("Ritual")
# Dano do ritual
# Frequência ou intervalo do ritual
# Cena

@export var ritual_damage: int = 1
@export var ritual_interval: float = 30
@export var ritual_scene: PackedScene

@export_category("Life")
@export var health: int = 100
@export var max_health: int = 100
@export var death_prefab: PackedScene

@export_category("Gold")
@export var gold_coins: int = 10

@onready var sprite: Sprite2D = $Sprite2D
@onready var animation_player: AnimationPlayer = $AnimationPlayer
@onready var sword_area: Area2D = $SwordArea
@onready var hitbox_area: Area2D = $HitboxArea
@onready var health_progress_bar: ProgressBar = $HealthProgressBar
@onready var heal_sound: AudioStreamPlayer = $HealSound
@onready var attack_sound: AudioStreamPlayer = $AttackSound
@onready var ritual_sound: AudioStreamPlayer = $RitualSound

@export_category("Movement")
var input_vector: Vector2 = Vector2(0, 0)
var is_running: bool = false
var was_running: bool = false
var is_attacking: bool = false
var attack_direction: String = "side"
# para permitir que o ataque seja executado mais vezes:
var attack_cooldown: float = 0.0
@export var speed: float = 3

var hitbox_cooldown: float = 0.0
var ritual_cooldown: float = 0.0


signal meat_collected(value: int)
signal gold_collected(value: int)

func _ready():
	# Contando para o gamemanager quem é o player
	GameManager.player = self
	meat_collected.connect(func(value: int): 
		GameManager.meat_counter += 1
	)
	gold_collected.connect(func(value: int):
		GameManager.gold_counter += 5
	)
	
	randomize()


func _process(delta):
	GameManager.player_position = position
	
	# Ler input
	read_input()
	
	# Processar ataque
	update_attack_cooldown(delta)
		# Ataque
	if Input.is_action_just_pressed("attack"):
		if Input.is_action_pressed("move_up"):
			attack("up")
		elif Input.is_action_pressed("move_down"):
			attack("down")
		else:
			attack("side")

	# Processar animação e rotação de sprite
	play_run_idle_animation()
	if not is_attacking:
		rotate_sprite()
		
	# Processar dano
	update_hitbox_detection(delta)
	
	# Ritual
	update_ritual(delta)
	
	# Atualizar health bar
	health_progress_bar.max_value = max_health
	health_progress_bar.value = health

func _physics_process(delta):	
	
	# Modificar a velocidade
	var target_velocity = input_vector * speed * 100.0
	if is_attacking:
		target_velocity *= 0.25
	velocity = lerp(velocity, target_velocity, 0.07)
	move_and_slide()

func update_attack_cooldown(delta: float):
		# Atualizar temporizador do ataque
	if is_attacking:
		attack_cooldown -= delta #toda vez que for atualizar o jogo, diminui o cooldown um poquinho (intervalo entre quadros) 0.6 - 0.016 = 0.584
		if attack_cooldown <= 0.0:
			is_attacking = false
			is_running = false
			animation_player.play("idle")
			
func update_ritual(delta: float):
	# Atualizar o temporizador
	ritual_cooldown -= delta
	if ritual_cooldown > 0: 
		return
	ritual_cooldown = ritual_interval
	
	# Criar ritual
	var ritual = ritual_scene.instantiate()
	ritual.damage_amount = ritual_damage
	# Colocar posição dentro do player
	add_child(ritual)
	
	# Tocar som do ritual
	if ritual_sound:
		print("Tocando som do ritual")
		ritual_sound.play()
	else:
		print("Som do ritual ou player de som não configurado")

func read_input():
		# Obter o input vecto
	input_vector = Input.get_vector("move_left", "move_right", "move_up", "move_down", 0.15)
	
		#Apagar dead zone do input vector
	# abs transforma qlq valor em positivo
	var deadzone = 0.15
	if abs(input_vector.x) < 0.15:
		input_vector.x = 0.0
	if abs(input_vector.y) < 0.15:
		input_vector.y = 0.0
		
	# Atualizar o is running
	was_running = is_running 
	is_running = not input_vector.length_squared() < 0.01

func play_run_idle_animation():	
	# Tocar animação
	if not is_attacking:
		if was_running != is_running:
			if is_running: 
				animation_player.play("run")
			else:
				animation_player.play("idle")
				
func rotate_sprite():
		# Girar Sprite
	if input_vector.x > 0:
		sprite.flip_h = false
		# Desmarcar flipH do Sprite2D
	elif input_vector.x < 0:
		# Marcar FlipH do Sprite2D
		sprite.flip_h = true

func attack(direction: String):
	if is_attacking: 
		return
	attack_direction = direction
	# Tocar som de ataque
	if attack_sound:
		attack_sound.play()
	# Tocar animação cima, baixo e lados
	if direction == "up":
		animation_player.play("attack_up_1")
	elif direction == "down":
		animation_player.play("attack_down_1")
	else:
		animation_player.play("attack_side_1")	
	
	# Configurar temporizador
	attack_cooldown = 0.6
	
	# Marcar ataque
	is_attacking = true
	
	
func deal_damage_to_enemies():
	var bodies = sword_area.get_overlapping_bodies()
	for body in bodies:
		# Checar se o body é inimigo:
		if body.is_in_group("enemies"):
			var enemy: Enemies = body
			# normalized deixa o vetor com 1 de comprimento
			sword_damage = randi() % 6 + 2
			
			if attack_direction == "side":
				var direction_to_enemy = (enemy.position - position).normalized() 
				var attack_direction_vector: Vector2
				if sprite.flip_h:
					attack_direction_vector = Vector2.LEFT
				else:
					attack_direction_vector = Vector2.RIGHT
				var dot_product = direction_to_enemy.dot(attack_direction_vector)
				if dot_product >= 0.3:
					enemy.damage(sword_damage)
			elif attack_direction == "up":
				if enemy.position.y < position.y:
					enemy.damage(sword_damage)
			elif attack_direction == "down":
				if enemy.position.y > position.y:
					enemy.damage(sword_damage)


func update_hitbox_detection(delta: float):
	# Temporizador
	hitbox_cooldown -= delta
	if hitbox_cooldown > 0: return
	
	# Frequência (2x por segundo)
	hitbox_cooldown = 0.5
	
	# Detectar inimigos
	var bodies = hitbox_area.get_overlapping_bodies()
	for body in bodies:
		if body.is_in_group("enemies"):
			var enemy: Enemies = body
			var damage_amount = 1
			damage(damage_amount)

func damage(amount: int):
	if health <= 0: return
	health-=amount
	print("Player recebeu dano de ", amount, " . A vida total é de ", health, "/", max_health)
	
	# Piscar inimigo / node
	modulate = Color.RED
	var tween = create_tween()
	tween.set_ease(Tween.EASE_IN)
	tween.set_trans(Tween.TRANS_QUINT)
	tween.tween_property(self, "modulate", Color.WHITE, 0.3)

	# Processar morte
	if health <= 0:
		die()

func die():
	GameManager.end_game()
	
	if death_prefab:
		var death_object = death_prefab.instantiate()
		death_object.position = position
		get_parent().add_child(death_object)
		
	print("Player morreu!")
	queue_free()

func heal(amount: int, sound: AudioStream = null) -> int:
	health += amount
	if health > max_health:
		health = max_health
	print("Player recebeu cura de ", amount, ". A vida total é de ", health, "/", max_health)
	if sound:
		heal_sound.stream = sound
		heal_sound.play()
	return health
	
func gold_bag_coins (gold_coins_amount: int) -> int:
	gold_coins += gold_coins_amount
	$CoinCollected.play()
	return gold_coins
